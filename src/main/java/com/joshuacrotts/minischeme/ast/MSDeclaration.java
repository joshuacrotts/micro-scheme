...
public abstract class MSDeclaration extends MSSyntaxTree {
    ...
    public MSDeclaration(final MSNodeType declType) {
        super(declType);
        if (!MSDeclaration.isValidDeclarationType(declType)) {
            throw new MSInterpreterException("Declaration must be of type read, proc, lambda, var, or let but got " + declType.toString());
        }
    }
    ...
    private static boolean isValidDeclarationType(final MSNodeType declType) {
        return declType == MSNodeType.DECL_READ || declType == MSNodeType.PROC_DECL
                || declType == MSNodeType.LAMBDA_DECL || declType == MSNodeType.VAR_DECL
                || declType == MSNodeType.EXPR_LAMBDA_DECL || declType == MSNodeType.LET_DECL;
    }
}
